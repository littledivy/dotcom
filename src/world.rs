...
pub enum WorldError {
  #[error(
    "extracting metadata from document produced no result or invalid result"
  )]
  TypstQuery,
  #[error("io error")]
  Io(#[from] io::Error),
  ...
  fn build_artifacts(
    ...
    route_tree
      ...
      .try_for_each(|(filename, node)| match node {
        ...
        RouteNode::Nested(hash_map) => {
          self.build_artifacts(hash_map, html_artifacts_path)
        }
    ...
    for node in routes.iter_mut() {
      match node {
        (slug, RouteNode::Redirect(path)) => {
          ...
          let redirect_template: Rendered<String> =
            create_redirect_page(&target_path);
        ...
        (slug, RouteNode::Page(doc)) => {
          ...
          let metadata =
            doc.metadata.as_ref().expect("no metadata in document");
