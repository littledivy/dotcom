...
pub fn run() {
  ...
  match &cli.command {
    Commands::Build(args) => {
      ...
      let dirs = match WorkingDirs::get_dirs() {
        Ok(dirs) => dirs,
        Err(err) => {
          tracing::event!(
            tracing::Level::ERROR,
      ...
      if args.watch {
        ...
        watcher
          .watch(Path::new("./routes"), notify::RecursiveMode::Recursive)
          .unwrap();

        std::mem::forget(watcher);
      }

      let the_world = World::from(dirs);
      ...
      for ev in rx {
        match ev {
          Ok(event) => {
            ...
            let the_world = World::from(WorkingDirs::get_dirs().unwrap());
            let routes = match the_world.get_routes() {
              Ok(routes) => routes,
              Err(err) => {
                tracing::event!(
            ...
            let site = Site::new(routes, move |slug, content, metadata| {
              ...
              match slug.as_str() {
                "/" => templates::Index
                  .render_page_with_content(raw_content, metadata),
                "/colophon" | "/impressum" | "/privacy" | "/photos"
                | "/charts" => templates::WidePage
                  .render_page_with_content(raw_content, metadata),
                _ => templates::Prose
                  .render_page_with_content(raw_content, metadata),
              }
            ...
            if let Err(err) = the_world.realize_site(site, args.minify) {
              ...
              tracing::event!(tracing::Level::INFO, "Rebuild complete.");
            }
          }
          Err(err) => {
            tracing::event!(tracing::Level::ERROR, "Watch error: {:?}", err)
          }
        }
      }
    }
